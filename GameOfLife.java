...
public class GameOfLife {
	...
	public static int[][] read(String fileName) {
		...
		while (!in.isEmpty()) {
			...
			for (int j = 0; j < s.length(); j++) {
				if(s!="") { 
					if(s.charAt(j) == 'x') board[line][j+1] =1;
    ...
    public static void print(int[][] arr) {
		for (int i = 1; i < arr.length-1; i++) { //  לא עשינו מינוס 1 כי רוצים להדפיס את הכול כולל האפסים בקצוות.
			for (int j = 1; j < arr[1].length-1; j++) {
				System.out.printf("%3s", arr[i][j]); //ה2 זה אומר רווח ואז התו וכך הלאה.
	    	}	
              System.out.println();
	    }
	}
